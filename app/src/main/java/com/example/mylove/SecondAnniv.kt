...
import com.example.mylove.databinding.FragmentSecondAnnivBinding

class SecondAnniv : Fragment() {

    private val itemList = mutableListOf<PictureData>()
    private lateinit var dbHelper: PictureDatabaseHelper
    private lateinit var adapter: PictureAdapter
    private lateinit var binding: FragmentSecondAnnivBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View{
        binding = FragmentSecondAnnivBinding.inflate(inflater, container, false)
    ...
    private fun loadVehicles(){
        try {
            itemList.clear()
            itemList.addAll(dbHelper.getPicturesByFragment("2nd Anniv"))
            adapter.notifyDataSetChanged()
        } catch (e: Exception) {
            Log.e("SecondAnniv", "Error loading pictures: ${e.message}")
        }
    }
