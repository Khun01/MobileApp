...
import com.example.mylove.databinding.FragmentFourthAnnivBinding

class FourthAnniv : Fragment() {

    private val itemList = mutableListOf<PictureData>()
    private lateinit var dbHelper: PictureDatabaseHelper
    private lateinit var adapter: PictureAdapter
    private lateinit var binding : FragmentFourthAnnivBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View{
        binding = FragmentFourthAnnivBinding.inflate(inflater, container, false)
    ...
    private fun loadVehicles(){
        try {
            itemList.clear()
            itemList.addAll(dbHelper.getPicturesByFragment("4th Anniv"))
            adapter.notifyDataSetChanged()
        } catch (e: Exception) {
            Log.e("FourthAnniv", "Error loading pictures: ${e.message}")
        }
    }
