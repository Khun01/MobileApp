...
import com.example.mylove.databinding.FragmentFirstAnnivBinding

class FirstAnniv : Fragment() {

    private val itemList = mutableListOf<PictureData>()
    private lateinit var dbHelper: PictureDatabaseHelper
    private lateinit var adapter: PictureAdapter
    private lateinit var binding: FragmentFirstAnnivBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View{
        binding = FragmentFirstAnnivBinding.inflate(inflater, container, false)
    ...
    private fun loadVehicles(){
        try {
            itemList.clear()
            itemList.addAll(dbHelper.getPicturesByFragment("1st Anniv"))
            adapter.notifyDataSetChanged()
        } catch (e: Exception) {
            Log.e("FirstAnniv", "Error loading pictures: ${e.message}")
        }
    }
